...
def replace_capitalized_tags(tag, conn_info):
    ...
    try:
        ...
        try:
            conn = connectToDatabase(conn_info, success_message = False)
            cursor = conn.cursor()
            cursor.execute("DELETE FROM tags WHERE keyword = (%s) AND id = (%s);", (tag.upper(), ID))
            conn.commit()
...
#        conn.close()
        ...
        while counts > 0:
            ...
            replace_capitalized_tags(tag, conn_info)
